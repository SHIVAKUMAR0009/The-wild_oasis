...
function SalesChart({ bookingsdata, numDays }) {
  const { darkMode } = useDarkMode();
  const alldays = eachDayOfInterval({
    start: subDays(new Date(), numDays),
    end: new Date(),
  }); ////////////////////////////////////////////generates array of date objects with a specified from and to

  const data = alldays.map((date) => {
    //////we looop over the array of date objects and return an new array with label total sales and extrasales of each day
    return {
      label: format(date, "MMM dd"),
      totalSales: bookingsdata /////to get the data of each day we loop over the bookings and for each booking we use issame day which looks for the datees each day if not found it returns 0 for that day if found we apply reduce method to calculate total sales if they are multiple bookings in day
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
        .reduce((acc, curr) => acc + curr.totalPrice, 0),

      extrasSales: bookingsdata
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
